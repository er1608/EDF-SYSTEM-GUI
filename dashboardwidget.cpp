...
void DashBoardWidget::setBorderColor(const QColor &color)
{
    m_borderColor = color;
    update();
}

void DashBoardWidget::setBorderWidth(int width)
{
    m_borderWidth = width;
    update();
}

void DashBoardWidget::setBorderRadius(int radius)
{
...
void DashBoardWidget::paintEvent(QPaintEvent *event)
{
    ...
    // VẼ VIỀN - THÊM ĐOẠN NÀY
    if (m_borderWidth > 0) {
        QPen borderPen(m_borderColor, m_borderWidth);
        painter.setPen(borderPen);
        painter.setBrush(Qt::NoBrush);
        painter.drawRoundedRect(
            rect().adjusted(m_borderWidth/2, m_borderWidth/2, -m_borderWidth/2, -m_borderWidth/2),
            m_borderRadius,
            m_borderRadius
            );
    }
