...
void MainWindow::setupPlotTab(QWidget *tab) {
  ...
  connect(loadCSVBtn, &QPushButton::clicked, this,
          [this, filePathLabel, columnCombo]() {
            ...
            if (!fileName.isEmpty()) {
              ...
              if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
                ...
                if (_logTextEdit) {
                  _logTextEdit->append(QString("[CSV] Loaded %1 rows from %2")
                                           .arg(_csvData.size())
                                           .arg(fileName));
                }
              }
  ...
  auto updateChartView = [this](const QVector<QPointF> &points, double startPos,
                                double visibleRange, QValueAxis *axisX,
                                QValueAxis *axisY, double yMin, double yMax) {
    _csvSeries->clear();
  ...
  connect(plotBtn, &QPushButton::clicked, this,
          [this, columnCombo, minLabel, maxLabel, avgLabel, pointsLabel, axisX,
           axisY, xScrollBar, updateChartView]() {
            if (columnCombo->currentIndex() < 0 || _csvData.isEmpty())
              return;
            ...
            if (validPoints > 0) {
              ...
              updateChartView(allPoints, 0, visibleRange, axisX, axisY, minVal,
                              maxVal);
              ...
              conn = QObject::connect(
                  xScrollBar, &QScrollBar::valueChanged, this,
                  [allPoints, xMin, totalRange, visiblePercentage, axisX, axisY,
                   minVal, maxVal, this](int value) {
                    double visibleRange = totalRange * visiblePercentage;
                    double startPos =
                        xMin + (totalRange - visibleRange) * (value / 100.0);

                    QVector<QPointF> visiblePoints;
                    for (const QPointF &point : allPoints) {
                      if (point.x() >= startPos &&
                          point.x() <= startPos + visibleRange) {
...
void MainWindow::setupTableTab(QWidget *tab) {
  ...
  _csvTableWidget->setStyleSheet("QTableWidget {"
  ...
  connect(loadTableBtn, &QPushButton::clicked, this, [this, tableInfoLabel]() {
    QString fileName = QFileDialog::getOpenFileName(
        nullptr, "Open CSV File for Table", QDir::homePath(),
        "CSV Files (*.csv);;All Files (*.*)");
      ...
      if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        if (!allLines.isEmpty()) {
          ...
          for (int row = 1; row < allLines.size(); row++) {
            ...
            for (int col = 0; col < qMin(values.size(), headers.size());
                 col++) {
          ...
          if (_logTextEdit) {
            _logTextEdit->append(QString("[TABLE] Loaded %1x%2 table from %3")
                                     .arg(_csvTableWidget->rowCount())
                                     .arg(_csvTableWidget->columnCount())
                                     .arg(fileName));
          }
