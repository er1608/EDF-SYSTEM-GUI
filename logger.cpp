...
{
    QString fileName = QDateTime::currentDateTime().toString("'Report_'yyyy-MM-dd_hh-mm-ss'.pdf'");
    ...
    painter.drawPixmap((pageWidth - imageWidth)/2, y, imageWidth, imageHeight, image);

    y += imageHeight + 150;

    QFont titleFont("Arial", 22, QFont::Bold);
    painter.setFont(titleFont);
    painter.drawText(QRect(0, y, pageWidth, 100),
                     Qt::AlignCenter,
                     "PROP TEST RESULT");
    ...
    painter.drawText(100, y, "Tài liệu bảo mật cấp A2  Bản quyền thuộc về CT UAV-CT Group  *Nghiêm cấm sao chép dưới mọi hình thức*");

    pdf.newPage();
    y = 200;

    painter.drawText(margin, y, "Ngày test: " + QDate::currentDate().toString("dd/MM/yyyy"));
    y += 60;
    ...
    painter.drawText(QRect(0, y, pageWidth, 50),
                     Qt::AlignCenter,
    ...
    QMessageBox::information(this, "Success", QString("PDF saved to %1").arg(fileName));
}
